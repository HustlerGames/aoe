...
void MovableObject::updateDirection(float dx, float dy)
{
	if (dx > 0)
	{
		if (dy < 0) direction = DIRECTION_UP_RIGHT;
		if (dy == 0) direction = DIRECTION_RIGHT;
		if (dy > 0) direction = DIRECTION_DOWN_RIGHT;
	}

	if (dx < 0)
	{
		if (dy < 0) direction = DIRECTION_UP_LEFT;
		if (dy == 0) direction = DIRECTION_LEFT;
		if (dy > 0) direction = DIRECTION_DOWN_LEFT;
	}
	...
	if (dx == 0)
	{
		...
		if (dy == 0) direction = DIRECTION_UP;
		if (dy > 0) direction = DIRECTION_DOWN;
	}
}
