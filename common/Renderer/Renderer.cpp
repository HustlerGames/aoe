...
void Renderer::drawLowObjects()
{
	...
	for (int x = 0; x < mapW; x++)
	{
		for (int y = 0; y < mapH; y++)
		{
			std::vector <Object*>* list = &world.map->cells[x][y].lowObjects;

			for (iter = list->end(); iter != list->begin(); iter--)
			{
				drawObject(*iter);
			}
		}
...
void Renderer::drawFence(BuildingFence* pFence)
{

	StateSpriteArray* res = (StateSpriteArray*)resources.pRoot
		->get(RESOURCE_CATEGORY_OBJECTS)
		->get(OBJECT_TYPE_BUILDING)
		->get(pFence->building_type);

	Frame* pFrame = &res->states[pFence->state].frames[pFence->appearence];

	WorldCoordinates coords = pFence->coordinates;
...
void Renderer::drawBuilding(Building* pBuilding)
{
	...
	{
		drawFence((BuildingFence*)pBuilding);
		return;
	}
...
void Renderer::drawHealth(Object* obj)
{
	if (obj->getType() == OBJECT_TYPE_UNIT)
	{
		...
		if (pUnit->health) {
			...
			GS.drawRectangle(
				coordinates.x - viewport.x + viewport.screen_x - 1,
				coordinates.y - viewport.y + viewport.screen_y - 1,
				coordinates.x - viewport.x + viewport.screen_x + 50 + 1,
				coordinates.y - viewport.y + viewport.screen_y + 10 + 1,
				BRUSH_BLACK_TRANSPARENT
			);
...
void Renderer::drawUnit(Unit* pUnit)
{
	...
	if (pUnit->pCurrentAction)
	{
		action = pUnit->pCurrentAction->type;
	}
...
void Renderer::drawObject(Object* pObject)
{
	...
	case OBJECT_TYPE_UNIT:
		drawUnit(dynamic_cast<Unit*>(pObject));
		break;
	case OBJECT_TYPE_BUILDING:
		drawBuilding(dynamic_cast<Building*>(pObject));
		break;
	case OBJECT_TYPE_SIMPLE:
		drawSimpleObject(dynamic_cast<SimpleObject*>(pObject));
		break;
	case OBJECT_TYPE_RESOURCE:
		drawResourceObject(dynamic_cast<ObjectResource*>(pObject));
		break;
	case OBJECT_TYPE_PROJECTILE:
		drawProjectileObject(dynamic_cast<Projectile*>(pObject));
	}
}
void Renderer::drawProjectileObject(Projectile* pObject)
{
	int direction = pObject->direction;
	Point coordinates = worldToCanvasCoords(&pObject->coordinates);

	DirectionAnimationResource* res = (DirectionAnimationResource*)resources.pRoot
		->get(RESOURCE_CATEGORY_OBJECTS)
		->get(OBJECT_TYPE_PROJECTILE)
		->get(pObject->projectile_type);

	Frame* pFrame = res->getFrame(direction, 0);

	drawFrame(res->bufferID, pFrame, coordinates.x, coordinates.y - pObject->h);
}
